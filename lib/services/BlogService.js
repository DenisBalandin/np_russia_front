...
class BlogService {
  ...
  static async getBlogBySearch(link) {
    try {
      const response = await fetch(
        `${config.server.url}/backend/search/${link}`
      );
      if (!response.ok) {
        throw new Error(`Error! status: ${response.status}`);
      }
  ...
  static async getBlogByCategory(link, page = 0) {
    try {
      const response = await fetch(
        `${config.server.url}/backend/category/${link}/${page}`
      );
      if (!response.ok) {
        throw new Error(`Error! status: ${response.status}`);
      }
  ...
  static async getBlogCategoryRowsCount(link) {
    try {
      const response = await fetch(
        `${config.server.url}/backend/cncount/${link}`
      );
      if (!response.ok) {
        throw new Error(`Error! status: ${response.status}`);
      }
